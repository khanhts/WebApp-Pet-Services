...
let { validate, validatorLogin, validatorForgotPassword, validatorChangePassword, validatorCreateUser} = require('../../utils/validators')
...
router.post('/signup', validatorCreateUser,validate, async function (req, res, next) {
    try {
        ...
        let newUser = await userController.CreateAnUser(
            body.username, body.password, body.email, 'customer'
        )
        CreateSuccessRes(res, jwt.sign({
            id: newUser._id,
            expire: (new Date(Date.now() + 60 * 60 * 1000)).getTime()
